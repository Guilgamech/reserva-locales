...
export const actions = {
    async listado({ commit }) {
        var has_error = false;
        const response = await this.$axios.$get('/medio/').
            catch(({ response }) => {
                has_error = true;
    ...
    async edit({ commit }, id) {
        let has_error = false;
        const response = await this.$axios.$get(`/medio/${id}/`).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS', response.status);
                commit('EXISTE', false);
    ...
    async save({ commit }, rol) {
        commit('CREANDO', true);
        let has_error = false;
        const res = await this.$axios.$post('/medio/', rol).
            catch(({ response }) => {
                has_error = true;
                commit('CREADO', false);
    ...
    unset_error({ commit }) {
        ...
        commit('SOLVED', "codigo");
        commit('SOLVED', "descripcion");
    },
    async patch({ commit }, { data, id }) {
        commit('EDITANDO', true);
        var has_error = false;
        const response = await this.$axios.$patch(`/medio/${id}/`, data).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS', response.status);
                if (response.status === 400) {
    ...
    async delete({ commit }, id) {
        ...
        var has_error = false;
        const response = await this.$axios.$delete(`/medio/${id}/`).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS', response.status);
                if (response.status === 400) {
        ...
        for (const property in err) {
            switch (property) {
                ...
                case "descripcion":
                    ...
                    break;
                case "codigo":
                    state.error.codigo = err[property];
                    break;
                default:
                    break;
            }
