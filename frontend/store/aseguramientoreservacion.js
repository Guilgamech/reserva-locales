...
export const actions = {
    async save({ commit }, id) {
        commit('CREANDO', true);
        let has_error = false;
        const res = await this.$axios.$post('/reservacion-aseguramiento/', id).
            catch(({ response }) => {
                has_error = true;
                commit('CREADO', false);
    ...
    unset_error({ commit }) {
        ...
        commit('SOLVED', "reservacion");
        commit('SOLVED', "aseguramiento");
    ...
    async patch({ commit }, { data }) {
        ...
        var has_error = false;
        let editdata = {
            aseguramiento_id: data.aseguramiento,
            cantidad: data.cantidad,
        }
        const response = await this.$axios.$put(`/reservacion/${data.reservacion}/modificar-aseguramiento/`, editdata).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS', response.status);
                if (response.status === 400) {
    ...
    async delete({ commit }, data) {
        ...
        var has_error = false;
        let deletedata = {
            aseguramiento_id: data.aseguramiento_id,
        }
        const response = await this.$axios.$post(`/reservacion/${data.reservacion_id}/eliminar-aseguramiento/`, deletedata).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS', response.status);
                if (response.status === 400) {
        ...
        for (const property in err) {
            switch (property) {
                case "cantidad":
                    ...
                    break;
                case "reservacion":
                    state.error.reservacion = err[property];
                    break;
                case "aseguramiento":
                    state.error.aseguramiento = err[property];
                    break;
                case "non_field_errors":
                    state.error.aseguramiento = ["el aseguramiento ya existe en este reservacion"];
                    break;
    ...
    SOLVED(state, property) {
        switch (property) {
            case "cantidad":
                ...
                break;
            case "reservacion":
                state.error.reservacion = [];
                break;
            case "aseguramiento":
                state.error.aseguramiento = [];
                break;
            case "non_field_errors":
