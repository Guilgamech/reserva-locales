...
export const actions = {
    async listado({ commit }) {
        var has_error = false;
        const response = await this.$axios.$get('/tipo-actividad/').
            catch(({ response }) => {
                has_error = true;
    ...
    async edit({ commit }, id) {
        let has_error = false;
        const response = await this.$axios.$get(`/tipo-actividad/${id}/`).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS', response.status);
                commit('EXISTE', false);
    ...
    async save({ commit }, rol) {        
        commit('CREANDO', true);        
        let has_error = false;
        const res = await this.$axios.$post('/tipo-actividad/', rol).
            catch(({ response }) => {
                has_error = true;
                commit('CREADO', false);
    ...
    async patch({ commit }, { data, id }) {
        ...
        var has_error = false;
        const response = await this.$axios.$patch(`/tipo-actividad/${id}/`, data).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS', response.status);
                if (response.status === 400) {
    ...
    async delete({ commit }, id) { 
        ...
        var has_error = false;
        const response = await this.$axios.$delete(`/tipo-actividad/${id}/`).
            catch(({ response }) => {
                has_error = true;
                commit('RESPONSE_STATUS',response.status);
                if (response.status === 400) {
